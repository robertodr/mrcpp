...
template <typename T, int D> class TensorTrain final {
    static_assert(std::is_floating_point_v<T>, "TensorTrain can only be instantiated with floating point types!");
...
private:
     ...
     */
    std::tuple<size_type, size_type> horizontal(std::size_t i) { return std::make_tuple(cores_[i].dimension(0), cores_[i].dimension(1) * cores_[i].dimension(2)); }

    /** Number of rows and columns of vertical unfolding of given core.
     *
     * @param[in] i index of the core
     */
    std::tuple<size_type, size_type> vertical(std::size_t i) { return std::make_tuple(cores_[i].dimension(0) * cores_[i].dimension(1), cores_[i].dimension(2)); }
    ...
    void decompose(T *A, size_type sz, T delta) {
        ...
        if (svd.info() != Eigen::Success) { MSG_ABORT("SVD decomposition of mode " << 0 << "(out of " << D << ") did not succeed!"); }
        ...
        for (int K = 1; K < D - 1; ++K) {
            ...
            if (svd.info() != Eigen::Success) { MSG_ABORT("SVD decomposition of mode " << K << "(out of " << D << ") did not succeed!"); }
     ...
     */
    template <int mode> auto to_full(const Eigen::Tensor<T, mode + 1> &pre, Eigen::Tensor<T, D> &full) const -> void {
     ...
     * @param[in,out] full the full format for `*this` tensor train.
     */
    template <> auto to_full<D - 1>(const Eigen::Tensor<T, D> &pre, Eigen::Tensor<T, D> &full) const -> void {
...
public:
     ...
     */
    TensorTrain(const std::array<size_type, D> &Is, const std::array<size_type, D + 1> &Rs)
            : modes_(Is) {
        for (auto K = 0; K < D; ++K) { cores_[K] = core_type(shape_type{Rs[K], modes_[K], Rs[K + 1]}).setZero(); }
    }
    ...
    void orthogonalize_RL() {
        ...
        for (auto i = D - 1; i > 0; --i) {
            ...
            matrix_type Ht = Eigen::Map<matrix_type>(cores_[i].data(), h_rows, h_cols).adjoint();
            ...
            // current, i-th, core
            cores_[i] = Eigen::TensorMap<core_type>(Qh.data(), shape_type{Qh.rows(), modes_[i], Qh.cols() / modes_[i]});

            // R factor (upper triangular)
            matrix_type R;
            if (do_thin_qr) {
                R = qr.matrixQR().topLeftCorner(h_rows, h_rows).template triangularView<Eigen::Upper>();
            } else {
            ...
            core_type next = cores_[i - 1].contract(Eigen::TensorMap<Eigen::Tensor<T, 2>>(R.data(), R.rows(), R.cols()), c_axes);
    ...
    void round(double epsilon) {
        ...
        for (auto i = 0; i < D - 1; ++i) {
            ...
            if (svd.info() != Eigen::Success) { MSG_ABORT("SVD decomposition of mode " << i << "(out of " << D << ") did not succeed!"); }
            ...
            core_type next = cores_[i + 1].contract(Eigen::TensorMap<Eigen::Tensor<T, 2>>(SigmaVh.data(), SigmaVh.rows(), SigmaVh.cols()), c_axes).shuffle(shuffle);
            // it seems it can't be done as: cores_[i+1] = cores_[i+1].contract(..)
            cores_[i + 1] = next;
        }
     ...
     */
    template <typename U> auto scale(U alpha) const -> TensorTrain<typename std::common_type<U, T>::type, D> {
        static_assert(std::is_floating_point_v<U>, "Scaling factor alpha can only be a floating point type!");
    ...
    auto ranks() const -> std::array<size_type, D + 1> {
        ...
        for (auto i = 0; i < cores_.size() - 1; ++i) { ranks[i + 1] = cores_[i].dimension(2); }
        return ranks;
    }
    /** Get i-th rank.
     *
    ...
    auto size() const -> size_type {
        return std::accumulate(cores_.cbegin(), cores_.cend(), 0, [](size_type sz, const core_type &Y) { return sz + Y.size(); });
    }
    ...
    auto uncompressed_size() const -> size_type { return std::accumulate(modes_.cbegin(), modes_.cend(), 1, std::multiplies<size_type>()); }

    /** Get size, in GiB, of uncompressed representation. */
    auto uncompressed_GiB() const -> double { return to_GiB<T>(uncompressed_size()); }

    /** Get compression rate. */
    auto compression() const -> double { return (1.0 - static_cast<double>(size()) / static_cast<double>(uncompressed_size())); }
    /*\@}*/
    /** Reconstruct full tensor from `*this` tensor train. */
    auto to_full() const -> Eigen::Tensor<T, D> {
        ...
        Eigen::Tensor<T, 3> post = (cores_[0].chip(0, 0)).contract(cores_[1], c_axes);
    ...
    template <typename U, typename V = typename std::common_type<T, U>::type> auto inner_product(TensorTrain<U, D> &Y) -> V {
 ...
 * @return Y scaled tensor train.
 */
template <typename T, typename U, int D> auto operator*(U alpha, const TensorTrain<T, D> &X) -> TensorTrain<decltype(U() * T()), D> {
 ...
 * @return Y scaled tensor train.
 */
template <typename T, typename U, int D> auto operator*(const TensorTrain<T, D> &X, U alpha) -> TensorTrain<decltype(T() * U()), D> {
    return X.scale(alpha);
}
 ...
 */
template <typename T, typename U, int D, typename V = typename std::common_type<U, T>::type> auto sum(const TensorTrain<T, D> &X, const TensorTrain<U, D> &Y) -> TensorTrain<V, D> {
    ...
    if (X.modes() != Y.modes()) { MSG_ABORT("X and Y tensors not compatible!"); }
 ...
 * @return Z the sum tensor train.
 */
template <typename T, typename U, int D, typename V = typename std::common_type<U, T>::type> auto sum(const TensorTrain<T, D> &X, const TensorTrain<U, D> &Y, double epsilon) -> TensorTrain<V, D> {
    auto Z = sum(X, Y);
 ...
 */
template <typename T, typename U, int D, typename V = typename std::common_type<U, T>::type> auto hadamard_product(const TensorTrain<T, D> &X, const TensorTrain<U, D> &Y) -> TensorTrain<V, D> {
    using size_type = typename TensorTrain<V, D>::size_type;
    ...
    for (auto i = 0; i < D; ++i) {
        ...
        for (auto j = 0; j < Z.mode(i); ++j) {
            ...
            Z.core(i).slice(offs, ext_Z) = X.core(i).slice(offs, ext_X).contract(Y.core(i).slice(offs, ext_Y), c_axes).shuffle(shuffle).reshape(ext_Z);
        }
    }
 ...
 * @return Z the elementwise product tensor train.
 */
template <typename T, typename U, int D, typename V = typename std::common_type<U, T>::type>
auto hadamard_product(const TensorTrain<T, D> &X, const TensorTrain<U, D> &Y, double epsilon) -> TensorTrain<V, D> {
