...
TEST_CASE("right-to-left orthogonalization of tensor train, with regular QR", "[tt][eigen][orthogonalize-RL][regular-QR]") {
    using shape_type = typename TensorTrain<double, 6>::shape_type;
    using core_type = typename TensorTrain<double, 6>::core_type;
    std::array<core_type, 6> cores = {core_type(shape_type{1, 5, 4}).setRandom(),
                                      core_type(shape_type{4, 8, 2}).setRandom(),
...
TEST_CASE("sum of two tensor trains, without rounding", "[tt][eigen][sum][no-rounding]") {
    auto A = sample_tensor<5, 5, 5, 5, 5, 5>();
...
TEST_CASE("Hadamard (elementwise) product of two tensor trains, without rounding", "[tt][eigen][hadamard][no-rounding]") {
    auto A = sample_tensor<5, 5, 5, 5, 5, 5>();
    ...
    // ranks are squared
    for (auto i = 0; i < 7; ++i) { REQUIRE(had_A.rank(i) == tt_A.rank(i) * tt_A.rank(i)); }
...
TEST_CASE("Hadamard (elementwise) product of two tensor trains, with rounding", "[tt][eigen][hadamard][rounding]") {
    auto A = sample_tensor<5, 5, 5, 5, 5, 5>();
...
TEST_CASE("inner product of two tensor trains", "[tt][eigen][inner]") {
    ...
    Eigen::array<index_pair_type, 6> cdims = {index_pair_type(0, 0), index_pair_type(1, 1), index_pair_type(2, 2), index_pair_type(3, 3), index_pair_type(4, 4), index_pair_type(5, 5)};
